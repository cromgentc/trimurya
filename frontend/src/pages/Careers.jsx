...
export default function Careers() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2">{filtered.map((job) => <article key={job.slug || job._id} className="rounded-2xl bg-white p-6 shadow-sm dark:bg-slate-900"><p className="text-sm font-bold text-secondary">{job.department || job.type || 'Job'}</p><h3 className="mt-2 text-xl font-black">{job.title}</h3><p className="mt-3 text-sm text-slate-500">{job.location || ''} • {job.type || ''}</p><Button className="mt-5"><FiUpload /> Apply Online</Button></article>)}</div>
    </section>
  );
}
